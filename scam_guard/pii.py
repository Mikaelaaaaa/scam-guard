...
# `[A-Z][12]` 加八位數字。**這個形狀同時涵蓋國民身分證、駕照與軍人補給證號**，
# 三者共用同一組字母對映與同一個加權和 checksum，在字元層面不可區分 ——
# 不是本模組分不出來，是它們本來就一樣。三者皆為個資，遮蔽的行為相同，
# 差別只在 `<TW_ID>` 這個標籤對後兩者是叫錯名字而不是遮錯東西。
# tw-PII-bench 上 `TW_ID` 的嚴格 precision 55.0%，差額幾乎全來自這件事。
#
# 前後的邊界條件不在規格裡，是本模組加的：嵌在更長的英數字串中間的十位數
# （`E73474605986` 這類寄件碼的子串）比較可能是單號而非證號，
# 而誤遮的代價大於漏遮。
