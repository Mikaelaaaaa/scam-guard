...
def test_an_unloaded_blocklist_does_not_degrade_the_status(client: TestClient) -> None:
    body = client.get("/health").json()
    assert body["blocklist"]["loaded"] is False
    assert body["status"] == STATUS_OK


def test_a_fresh_blocklist_is_ok(frozen_today: None) -> None:
    store = _FakeStore({"176455": _source(FRESH)})
    health = build_health(REGISTRY, UNREGISTERED_CHECKS, store, 7, TABLE)
...
def test_freshness_is_recomputed_on_every_call(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    at_startup = build_health(REGISTRY, UNREGISTERED_CHECKS, store, 7, TABLE)
    assert at_startup.status == STATUS_OK

    monkeypatch.setattr(api.health, "utc_today", _FrozenToday(date(2026, 9, 20)))
    later = build_health(REGISTRY, UNREGISTERED_CHECKS, store, 7, TABLE)
...
def test_the_threshold_comes_from_the_assembly_layer(frozen_today: None) -> None:
    """新鮮度門檻是部署參數，不是核心可以替我們決定的事。"""
    store = _FakeStore({"176455": _source(FRESH)})
    assert build_health(REGISTRY, UNREGISTERED_CHECKS, store, 3, TABLE).status == STATUS_DEGRADED
    assert build_health(REGISTRY, UNREGISTERED_CHECKS, store, 30, TABLE).status == STATUS_OK
